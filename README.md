...
- `dialogue_texture` 测量对白自身的形状：句数、长度中位数与均值、≤6 字与 ≥30 字占比、句长起伏（p80−p20），以及按说话人聚合的长度与分句统计。分两档：与说话人归属无关的数字触发 `dialogue_clipped`（全章只剩确认与应答）与 `dialogue_monotone`（不论谁说、说什么长度都在同一档）两类**提示**；按说话人聚合的 `voiceDistance` 只作为终审参考，不下判断。**从不拦截**。
- 说话人归属优先使用项目角色卡里的真实姓名；没有名册时回落到「X说」提示语解析，会漏掉代词与无提示语的台词，`attributedLines` 偏低不代表对白少。归属只用于提供参考数字，"谁在说分不分得出来"始终由终审模型判断，不由规则判断。

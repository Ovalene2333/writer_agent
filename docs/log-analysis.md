...
OpenCode 需先自行完成 provider 认证。模型 ID 使用 `provider/model` 格式：
...
OpenCode 的工作目录被限制在单次审计目录，`plan` Agent 只负责读取证据并输出 JSON。WSL 中如果
Windows npm shim 不可用，可用 `--opencode-bin` 指向 Linux 原生安装，或启动 `opencode serve` 后传

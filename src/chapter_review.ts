...
export type ChapterReviewInput = {
  ...
  context?: string;
  /** Review-only openings from nearby prose; never prose-model style evidence. */
  comparisonMaterials?: Array<{
    path: string;
    content: string;
    role: "template_check";
  }>;
  ...
  for (const row of rows) {
    ...
    const evidence = Array.isArray(issue.evidence)
      ? issue.evidence.map(item => boundedString(item, 180))
        ...
        .slice(0, 3)
      : [];
    if ((kind === "field_verbalization" || kind === "voice_macro_reuse" || kind === "template_reuse")
      && evidence.length < 2) continue;

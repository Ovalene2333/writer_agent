...
function competency(v: unknown): CharacterCompetency { const r = obj(v); return { id: txt(r.id), name: txt(r.name), summary: txt(r.summary), level: txt(r.level), unlocked: r.unlocked === true, description: txt(r.description), resources: strs(r.resources), limitations: strs(r.limitations), costs: strs(r.costs), ...temporal(r) }; }

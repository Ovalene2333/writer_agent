...
一场戏从人物此刻想完成的事情生长。贴住视角人物会注意、误会、回避和确认的内容；让行动得到回应，并在局面、理解、关系或选择中留下变化。关键瞬间可以放慢，过门可以轻，静场也允许只完成细微但真实的移动。

对白是人物对彼此采取的行动。直说、解释、回避、沉默、玩笑或让步都可以，只要符合说话者的知识、目的与关系；人物差异来自各自在意、默认和不愿承认的东西，不靠强塞口头禅。

环境不是布景清单。只写那些被人物利用、躲避、误读，或会改变身体感受和下一步动作的细节。情绪不要先命名再证明，而要落在注意力偏移、动作时机、身体失误、过度克制和没有说出口的话里。允许读者比人物早一步或晚一步明白，允许段落在余波尚未解释时结束。

具体事实通过人物能接触的证据、判断和后果进入现场。量化、技术原理与状态信息在人物需要据此行动时可以充分展开；与当下选择无关的部分留在背景里。
...
export function buildIsolatedSceneWriterMessages(
  ...
  if (oppositionMove || sceneCost || readerQuestion) {
    sections.push([
      ...
      sceneCost ? `场景材料预期这次变化伴随“${sceneCost}”。让它在人物选择与后果中成立；若现场因果表明它会转化或部分挽回，以真实结果为准。` : "",
      readerQuestion ? `材料希望读者继续关心“${readerQuestion}”。让正文保留这份牵引；若本场目标要求回答它，就用答案自然引出新的条件，而非机械悬置。` : "",
    ].filter(Boolean).join(""));
  }

  const mustLand = compactLines(input.writePack.mustLand);
  if (mustLand.length) {
    sections.push(`正文还需要自然留下这些可被读者察觉的事实：${naturalClause(mustLand)}。让它们在最合适的动作、对白或后果中出现，不必挤进同一个节拍。`);
  }
  ...
  if (avoidNotes.length) {
    sections.push(`已写正文中出现了这些可能重复的表达倾向，写本场时作为轻量提醒；若与人物语气或现场需要冲突，以正文自然成立为准：\n${avoidNotes.map(note => `- ${note}`).join("\n")}`);
  }

...
export async function generateCharacter(input: {
  ...
  const messages: ToolLoopMessage[] = [
    { role: "system", content: `你是小说角色设计助手。只输出 schema v3 JSON 对象，不要 Markdown。顶层字段为 identity/profile/psychology/motivations/voice/features/competencies/storyStates/experiences/notes；profile 固定使用 appearance/appearanceSummary/background/backgroundSummary/biography，其中 appearance 与 background 放完整资料，两个 Summary 以换行分隔要点、每行一项，不限定固定总字数。结构化条目必须有稳定 ASCII id，演进记录可包含 status/validFrom/validUntil。voice 描述角色在对白中如何选择信息、回应别人、处理关系和压力变化；句子长短、口头禅和示例只是佐证，不要把碎句写成角色本质或叙述风格。features 用于不属于能力但会影响描写的稳定细节，每项填写 name、summary、description，summary 使用自然短段落。competencies 只保存稳定能力定义，每项填写 name、summary、description、resources、limitations、costs；unlocked 是旧格式兼容值，不得用它独自表达觉醒、封印或失去。剧情时点的能力可用性写入 storyStates[].competencyStates，每项包含 id、competencyId、state(available|latent|blocked|lost|unknown)、reason，可选 evidence；只有资料明确支持时才能把 unknown 改成更具体的状态，失败尝试不能自动变成 available。experiences 为已确认经历条目（id/label/description，可选 validFrom），不是 biography 散文。只填写用户已提供或可可靠归纳的事实，未知内容留空；不要自行拆解或补写事实，不要输出 relationships。identity.name 必须提供。` },
